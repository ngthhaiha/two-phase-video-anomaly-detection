...
def plot_model(rows, model_name: str, out_dir: Path):
    ...
    ys = [r["Macro F1"] for r in model_rows]

    ymin = max(0.0, min(ys) - 0.025)
    ymax = min(1.0, max(ys) + 0.025)

    fig, ax = plt.subplots(figsize=(6.0, 4.0), dpi=200)
    ax.plot(xs, ys, marker="o", linewidth=2.2, markersize=6.5, color="#2563eb")
    ...
    ax.set_title(f"{model_name}: Top-N vs Macro-F1", fontsize=12, pad=10)
    ax.set_xlabel("Top-N")
    ...
    out_path = out_dir / f"macro_f1_{MODEL_SLUGS[model_name]}_N6_8_10_12.png"
